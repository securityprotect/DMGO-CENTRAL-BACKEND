...
export async function POST(req: Request) {
  ...
  let webhookStatus: 'processed' | 'failed' = 'processed';
  let webhookError = '';
    ...
    for (const change of entry?.changes || []) {
      ...
      for (const automation of activeAutomations) {
        ...
        try {
          if (automation.replyMode === 'comment_and_dm' && commentReplyText) {
            ...
            await safeApiLog({
              ...
              requestPayload: { commentId, message: commentReplyText },
            });
          }
          if (automation.sendDm && commenterId && dmText) {
            ...
            await safeApiLog({
              ...
              statusCode: 200,
              durationMs: Date.now() - apiStartedAt,
              requestPayload: { commentId, message: dmText },
            });
          }
          status = 'sent';
        } catch (error) {
          ...
          await safeErrorLog({
            ...
            retryPayload: { commentId, commenterId, automationId: automation._id },
          });
          await safeApiLog({
            userId: account.userId,
            service: 'Instagram API',
            ...
            requestPayload: { commentId, commenterId, dmText },
            errorMessage: failReason,
          });
        ...
        const activity = await Activity.create({
          ...
          failReason,
          rawPayload: body,
          responsePayload: { commentId, commenterId, status, failReason },
          durationMs: Date.now() - startedAt,
        });
        await AutomationLog.create({
          ...
          outgoingApiRequest: { commentId, commenterId, dmText, commentReplyText },
          ...
          executionFlow: [
            ...
            { label: 'AI Generated Response', status: 'skipped', detail: 'Template reply' },
            { label: 'DM API Called', status: status === 'sent' ? 'ok' : 'failed' },
            { label: 'Success / Failed', status },
          ],
          executionDurationMs: Date.now() - startedAt,
          errorMessage: failReason,
          rawPayload: body,
          responsePayload: { activityId: String(activity._id), status },
        }).catch((error) => console.error('[AUTOMATION_LOG] create failed', error));
        await safeQueueJob({
          queueName: 'dm-send',
          userId: account.userId,
          jobType: 'Send DM',
          status: status === 'sent' ? 'completed' : 'failed',
          payload: { activityId: String(activity._id), automationId: automation._id },
  ...
  await WebhookLog.create({
    ...
    responseCode: webhookStatus === 'processed' ? 200 : 500,
    processingTimeMs: Date.now() - startedAt,
    headers: Object.fromEntries(req.headers.entries()),
    rawPayload: body,
    responsePayload: { ok: webhookStatus === 'processed' },
    replayable: true,
    errorMessage: webhookError,
  }).catch((error) => console.error('[WEBHOOK_LOG] create failed', error));
