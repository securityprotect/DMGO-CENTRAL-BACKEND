...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error('[auth:forgot-password:verify-otp] database connection failed', error);
    if (isJson) {
    ...
    return NextResponse.redirect(
      new URL(`/sign-up-login-screen?view=forgot&sent=1&email=${encodeURIComponent(email)}&error=db_unavailable`, origin),
      { status: 303 }
    );
  }
